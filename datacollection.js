;(function() {
  ...
  function ajax(option) {
    ...
    pram.append('adoptToken', 'F0F3854D68D9773D774E9FAD166A4D73')
    pram.append('data', JSON.stringify(data))

    ajax.send(pram)
    ajax.onreadystatechange = function() {
      if (ajax.readyState === 4) {
        if (ajax.status >= 200 && ajax.status < 300 || ajax.status === 304) {
          option.success(JSON.parse(ajax.responseText))
        } else {
          option.error()
        }
      }
  ...
  ajax({
    url: 'http://192.168.1.21:9971/receive/browse',
    data: postData,
    method: 'post',
    success: (res) => {
      if (res.code === 200) {
  ...
  eventMonitor.setClickNum = function() {
    ajax({
      url: 'http://192.168.1.21:9971/receive/click',
      data: {
        clickNum: 1
      },
      method: 'post',
